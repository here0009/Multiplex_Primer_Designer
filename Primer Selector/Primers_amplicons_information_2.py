...
IUPAC_DICT = {'R':{'A','G'}, 'Y':{'C','T'},'S':{'G','C'},'W':{'A','T'},
              ...
              'H':{"A","C","T"},'V':{"A","C","G"},'N':{"A","T","C","G"},'I':{"A","T","C","G"},
             "X":{"A","T","G","C"}, 'A':{'A'}, 'G':{'G'}, 'T':{'T'}, 'C':{'C'}}

degenerate_number_dict = {'R':2, 'Y':2,'S':2,'W':2,'K':2,'M':2,'B':3,'D':3,'H':3,'V':3,'N':4,"X":4, 'I':2}
...
class DegeneratePrimer:
    def __init__(self, name, sequence):
        ...
        self.len = len(sequence)
        self.degenerate_number = seq_degenerate_number(self.sequence)
        self.adjusted_deg_num = np.ceil(seq_adjusted_deg_num(self.sequence))
...
def seq_adjusted_deg_num(seq):
    '''
    return the adjusted degenerate number of the given seq_conversion
    '''
    adjusted_degenerate_number = 1
    positon_factor = 1.2 #if index%5 == 0, position_factor -= 0.2
    reverse_seq = seq[::-1]
    for index, letter in enumerate(reverse_seq):
        if index%5 == 0 and  positon_factor > 0.2:
            positon_factor -= 0.2
        if letter in degenerate_number_dict:
            temp_deg_number = (degenerate_number_dict[letter] - 1) * positon_factor + 1 
            #degnumber is 4, temp_deg_number = (4-1) * 1 + 1
            adjusted_degenerate_number = adjusted_degenerate_number * temp_deg_number
    return adjusted_degenerate_number 
...
def read_fasta_file(file_name) :
    ...
    number = 1
    # with open (file_name, encoding='utf-8', errors='ignore') as fhand:
...
def cpl(seq):
    ...
    '''
    complementary_dict ={'A':'T', 'T':'A', 'G':'C', 'C':'G','Y':'R', 'R':'Y', 'W':'W', 'S':'S', 'K':'M', 'M':'K', 'D':'H', 'H':'D', 'V':'B', 'B':'V', 'X':'X', 'N':'N', 'I':'I'}
    try:
...
def primer_information_check(primer_name, primer_seq,file_hand):
    flag = True
    msg_deg = '{:35}\t{:40}\t{:^10d}\t{:^10d}\t{:^5.2f}\t{:^5.2f}\t{:^5.2f}\t{:^10d}\n' #for output of degenerate primers
    ...
    if max_tm >degenerate_max_tm or min_tm<degenerate_min_tm or primer.degenerate_number>max_degenerate_number :
        flag = False
    file_hand.write(msg_deg.format(primer_name,primer_seq,primer.len,primer.degenerate_number,primer.adjusted_deg_num,max_tm,min_tm, flag))
    return flag
...
def primer_info_doc(file_tag):
    ...
    headline = '{:35}\t{:40}\t{:^10}\t{:^10}\t{:^10}\t{:^5}\t{:^5}\t{:^10}\n'
    fhand_primer_information.write(headline.format('Name','Sequence','Len','DegNumber','AdjuDegNum','MaxTm','MinTm','Flag'))
    for primer_name, primer_seq in primer_dict.items():
