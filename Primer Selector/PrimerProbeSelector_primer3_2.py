...
#modifiction 20190917: 
#1.the probe region include forward and reverse sequence.
#2. the probe name is named as ref_name@f/r@start_index@end_index
#3. add the function to calculate the G:C ratio of a probe, if G of probe is larger than C, remove the probe.
#modification 20171026:
#4. add the function to remove the probe have 4~5 continue G
...
degenerate_max_tm = 63 #The max Tm of degenerate primers, Celsius Degree
degenerate_min_tm = 45 #The min Tm of degenerate primers, Celsius Degree
max_degenerate_number = 32 #The max degeneration number of primer
...
rule_7_cutoff = -7 #delta G cutoff score, kcal/mol
...
probe_ambigous_letter_threshold = 12 #The max ambigous letter allowed in probe.
...
hairpin_tm_threshold = 60 #The max tm allowed for a hairpin structure, default 50
homodimer_dg_threshold = -9000 #The min deltaG value allowed for homodimer structure, cal/mol
probe_G_check = True #If set to true, it will check the probe_more_GtoC_ratio and probe_max_G_num
probe_GtoC_ratio_threshold = 1.2 #the ratio of G to C of a probe, should not exceed this value, default is 1.2
probe_G_ratio_threshold = 0.2 #the ratio of G in probe should be no more than this value, default is 0.2
probe_max_deg_threshold = 24 #The maximum degeneration number allowed for probe
probe_ambigous_letter_threshold = 4 #The max ambigous letter allowed in probe.
probe_ctn_G_check = True #If set to ture, the script will check the max continouse G, make sure it is not equal or larger than probe_ctn_G_cutoff
probe_ctn_G_cutoff = 4 #the max number of continous G allowed
seperate_line = "#####################################\n"
...
def probe_init_G(seq):
    ...
    if 'G' in IUPAC_DICT[init_letter]:
        ...
        return False
def probe_ctn_G_num(seq):
    '''
    Return the largest continue G number of a probe
    '''
    ctn_G_num = 0
    max_ctn_G_num = 0
    for letter in seq:
        if 'G' in IUPAC_DICT[letter]:
            ctn_G_num += 1
            if ctn_G_num > max_ctn_G_num:
                max_ctn_G_num  = ctn_G_num
        else:
            ctn_G_num = 0
    return max_ctn_G_num
...
def nucleotide_number(seq, nucleotide):
    '''
    Return the number of letter in seq, letter is limited to 'ATGC'
    '''
    if nucleotide not in ATGC:
        raise KeyError("undefiend letter")
    else:
        number = 0
        for letter in seq:
            if letter == nucleotide:
                ...
                letters = IUPAC_DICT[letter]
                if nucleotide in letters:
                    number += 1/len(letters)
    return number
...
def primer_alignment_write(primer_1, primer_2, partial_or_full_len, parameter, fhand):
    '''
    A function return the alignment of two primers.
    Input: two primer sequences, 
        partial_or_full_len : means the state of primer_1 and primer_2 alignment
    ...
    if partial_or_full_len == 'partial':
        ...
        fhand.write('Warning : Wrong Input value of partial_or_full_len\n')
        return
    #Convert the list to string
    for i in range(len_seq):
        if primer_1_print[i] == cpl(primer_2_print[i]):
    ...
    fhand.write (primer_1_print+'\n')
    fhand.write (middle_line+'\n')
    fhand.write (primer_2_print+'\n')
...
def probe_region_generation(primer_list, ref_dict, position_dict,fhand):
    ...
    probe_region_dict = dict()
    probe_region_cord = defaultdict(list) #store the coordination of the probe on ref
    fhand.write(seperate_line)
    for primer_name in primer_list:
        ...
        if position_info.strand == 'forward':
            probe_start_index = position_info.pos_end + primer_probe_gap
            #Change the probe region, trime the primer_seq + primer_probe_gap from ref_seq
            probe_region_cord[position_info.ref_name].append(probe_start_index) 
        elif position_info.strand == 'reverse':
            probe_end_index = position_info.pos_start - primer_probe_gap
            probe_region_cord[position_info.ref_name].append(probe_end_index)

        fhand.write('{}\t{}\t{}\t{}\n'.format(position_info.primer_name, position_info.ref_name, position_info.pos_start,position_info.pos_end))

    fhand.write('\nThe probes range are:\n')
    for ref_name, probe_cord in probe_region_cord.items():
        #Add probe region f/r to probe_region_dict
        ref_name_f = ref_name + '@f'
        ref_name_r = ref_name + '@r'
        probe_cord = sorted(probe_cord) #make the smaller index at begining
        probe_region_f =  ref_dict[ref_name][probe_cord[0]:probe_cord[1]]
        probe_region_r = rev_cpl(probe_region_f)

        fhand.write('{}\t{}\t{}\n'.format(ref_name,probe_cord[0],probe_cord[1]))
        probe_region_dict[ref_name_f] = probe_region_f
        probe_region_dict[ref_name_r] = probe_region_r

    return probe_region_dict

def probe_candi_dict_generation(probe_region_dict, probe_info_fhand, probe_interaction_fhand, probe_info, primer_dict, primer_list):
    '''
    ...
    for ref_name, ref_seq in probe_region_dict.items():
        for probe_len in range(probe_len_min, probe_len_max+1):
            for i in range(len(ref_seq) - probe_len):
                start_index = str(i)
                end_index = str(i+probe_len)
                probe_name = ref_name + '@' + start_index + '@' + end_index
                probe_seq = ref_seq[i:i+probe_len]
                if probe_check(probe_name, probe_seq, probe_info_fhand, probe_interaction_fhand, probe_info, primer_dict, primer_list):
...
def probe_check(probe_name, probe_seq, fhand_information, fhand_interaction, probe_info, primer_dict, primer_list):
    '''
    ...
    if probe_ctn_G_check : #this parameter is true, check if the max continouse G number larger than the cutoff value
        if probe_ctn_G_num(probe_seq) >= probe_ctn_G_cutoff:
            return False
    ...
    probe_G_number = nucleotide_number(probe_seq, 'G')
    probe_C_number = nucleotide_number(probe_seq, 'C')
    probe_G_ratio = probe_G_number/len(probe_seq)
    probe_GtoC_ratio = probe_G_number/probe_C_number
    if probe_G_check:
        if probe_G_ratio > probe_G_ratio_threshold and probe_GtoC_ratio > probe_GtoC_ratio_threshold:
            return False
    ...
    degenerate_primers_0 = degenerate_probe.atcg_sequence()
    # for seq in degenerate_primers_0:
    #     fhand_interaction.write(seq+'\n')

    for primer_name in primer_list:
        primer_seq = primer_dict[primer_name]
        degenerate_primers_1 = DegeneratePrimer(primer_name,primer_seq).atcg_sequence()

        for primer_atcg_comb in itertools.product(degenerate_primers_0,degenerate_primers_1):

            if probe_primer_check(primer_atcg_comb[0],primer_atcg_comb[1], fhand_interaction) == False:
                fhand_interaction.write("{} and {} may have interaction\n".format(probe_name,primer_name ))
                fhand_interaction.write('\n')
                # break
                return False

    else:
        fhand_information.write(probe_info.format(probe_name, probe_seq, ambigous_number, degenerate_number, tm_min, tm_max, hairpin_number, max_hairpin_tm, hairpin_ratio, homodimer_number, min_homo_dimer_dg, homodimer_ratio, probe_G_ratio, probe_GtoC_ratio))
        return True

def probe_primer_check(primer_1, primer_2, fhand):
    '''
    Check the interation between the primer and probe, probe do not have to check 3' interaction, so just rule 3 to 7 is checked. Using the primer parameter temporarily
    primer_1 is the probe sequence
    primer_2 is the primer sequence
    '''
    ...
    primer_2 = str.upper(primer_2)
    if len(primer_1) < len(primer_2):
        primer_1, primer_2 = primer_2, primer_1 # make sure primer_1 is longer 
    rev_cpl_primer_2 = rev_cpl(primer_2) #change primer 2 to reverse complementary sequence for analysis
    panelty_score = 0
    ...
    rule_3, flag, parameter = primer_score(primer_1, rev_cpl_primer_2, match_score,0,rule_3_len) #0 means no gap, 7 is the min len
    if rule_3 >= rule_3_cutoff:
        fhand.write (msg.format(primer_1, primer_2, 3, rule_3 ))
        primer_alignment_write(primer_1, primer_2, flag, parameter, fhand)
        panelty_score += 1

    rule_4, flag, parameter = primer_score(primer_1, rev_cpl_primer_2, match_score,1,rule_4_len)#1 means 1 gap, 7 is the min len
    if rule_4 >= rule_4_cutoff:
        fhand.write (msg.format(primer_1, primer_2, 4, rule_4))
        primer_alignment_write(primer_1, primer_2, flag, parameter, fhand)
        panelty_score += 1

    rule_5, flag, parameter = primer_score(primer_1, rev_cpl_primer_2,match_percents, rule_5_len)
    if rule_5 >= rule_5_cutoff:
        fhand.write (msg_float.format(primer_1, primer_2, 5, rule_5))
        primer_alignment_write(primer_1, primer_2, flag, parameter, fhand)
        panelty_score += 1
    ...
    rule_6, flag, parameter = primer_score(primer_1, rev_cpl_primer_2, alignment_score,rule_6_len) #8 is the min len
    if rule_6 >= rule_6_cutoff:
        fhand.write (msg.format(primer_1, primer_2, 6, rule_6))
        primer_alignment_write(primer_1, primer_2, flag, parameter, fhand)
        panelty_score += 1
    ...
    if rule_7 >= rule_7_cutoff:
        rule_7 = -1*rule_7 #change the score back to negative value.
        fhand.write (msg_float.format(primer_1, primer_2, 7, rule_7))
        primer_alignment_write(primer_1, primer_2, flag, parameter, fhand)
        panelty_score += 1

    
    if panelty_score >= 1:
        fhand.write ('The panelty_score is {}\n'.format(panelty_score))
        return False
    else:
        return True

def primer_combinations(primer_combinations_file,outputfile_probe_info, outputfile_probe_interaction, group_name_dict, confliction_table, primer_amplicon_match_dict, position_dict, primer_dict, ref_dict):
    '''
    ...
    #file hand to store the probe information
    probe_info = '{:35}\t{:35}\t{:^4}\t{:^4}\t{:^.2f}\t{:^.2f}\t{:^4}\t{:^.2f}\t{:^.2%}\t{:^4}\t{:^.2f}\t{:^.2%}\t{:^.2f}\t{:^.2f}\n'
    probe_info_header = '{:35}\t{:35}\t{:^4}\t{:^4}\t{:^4}\t{:^4}\t{:^4}\t{:^4}\t{:^4}\t{:^4}\t{:^4}\t{:^4}\t{:^4}\t{:^4}\n'
    probe_info_fhand = open(outputfile_probe_info, 'w')
    probe_info_fhand.write(probe_info_header.format('Probe_Name', 'Probe_Seq', 'Ambigous_Number', 'Deg_Number', 'Tm_min', 'Tm_max', 'Hairpin_Number', 'Max_Hairpin_Tm', 'Hairpin_Ratio','Homodimer_number', 'Min_HomoDimer_dg','HomoDimer_Ratio','G_ratio','GtoC_ratio'))
    #file hand to store the probe-primer interaction
    probe_interaction_fhand = open(outputfile_probe_interaction, 'w')
    ...
    with open(primer_combinations_file,'w') as fhand:
        ...
        for product in itertools.product(*group_name_dict.values()):
            ...
            if flag == False: #no interaction, write the primers to the file
                ...
                probe_region_dict = probe_region_generation(primer_list, ref_dict, position_dict, fhand)

                #Write probe region dict to file
                fhand.write("Probe region dict are:")
                fhand.write('\n')
                for name, seq in probe_region_dict.items():
                    fhand.write(name+'\n')
                    fhand.write(seq+'\n')
                fhand.write('\n')

                probe_candi_dict = probe_candi_dict_generation(probe_region_dict, probe_info_fhand,  probe_interaction_fhand, probe_info, primer_dict, primer_list)

                fhand.write('\n')
                fhand.write("The probe can be used for the primers are: \n")
                for ref_name, probe_seqs in sorted(probe_candi_dict.items()):
...
def main():
    ...
    outputfile_probe_interaction = ref_file.split(".")[0] + "_probes_interaction.txt"
    primer_combination_file = primer_file.split(".")[0]+'_primer_probe_combinations.txt'
    primer_combinations(primer_combination_file, outputfile_probe_info, outputfile_probe_interaction,group_name_dict, confliction_table, primer_amplicon_match_dict, position_dict, primer_dict, ref_dict)
