...
rule_7_cutoff = -8 #delta G cutoff score, kcal/mol
...
primer_probe_gap = 1 #the nucleotides between primer and probe, default set to be 1, the probe is next to the primer
probe_len_min = 24
probe_len_max = 32
probe_tm_min = 55
probe_tm_max = 80
probe_init_noG_check = False #If set to true, the 5' nucleotide of the primer should not be G
probe_G_check = True #If set to true, it will check the probe_more_GtoC_ratio and probe_max_G_num
probe_GtoC_ratio_threshold = 1.4 #the ratio of G to C of a probe, should not exceed this value, default is 1.2
probe_G_ratio_threshold = 0.25 #the ratio of G in probe should be no more than this value, default is 0.2
probe_max_deg_threshold = 24 #The maximum degeneration number allowed for probe
probe_ambigous_letter_threshold = 5 #The max ambigous letter allowed in probe.
...
probe_ctn_G_cutoff = 5 #the max number of continous G allowed
...
def probe_check(probe_name, probe_seq, fhand_information, fhand_interaction, probe_info, primer_dict, primer_list):
    ...
    for primer_name in primer_list:
        ...
        fhand_information.write(probe_info.format(probe_name, probe_seq, len(probe_seq),ambigous_number, degenerate_number, tm_min, tm_max, probe_G_ratio, probe_GtoC_ratio))
        return True
...
def primer_combinations(primer_combinations_file,outputfile_probe_info, outputfile_probe_interaction, group_name_dict, confliction_table, primer_amplicon_match_dict, position_dict, primer_dict, ref_dict):
    ...
    probe_info_header = '{:35}\t{:35}\t{:4}\t{:^4}\t{:^4}\t{:^4}\t{:^4}\t{:^4}\t{:^4}\n'
    probe_info = '{:35}\t{:35}\t{:4}\t{:^4}\t{:^4}\t{:.2f}\t{:.2f}\t{:.2f}\t{:.2f}\n'
    probe_info_fhand = open(outputfile_probe_info, 'w')
    probe_info_fhand.write(probe_info_header.format('Probe_Name', 'Probe_Seq', 'Probe_len','Ambigous_Number', 'Deg_Number', 'Tm_min', 'Tm_max','G_ratio','GtoC_ratio'))
...
# def test():
#     test_seq = {'ACTTGGCCGGGGGGCCCC', 'NNNNNNNGGCC','AGGGGGGGCCCCCC','SSSSSSGGGSS','AAGGCCCCCCC'} 
#     for seq in test_seq:     
#         print(seq, len(seq), probe_ctn_G_num(seq))
