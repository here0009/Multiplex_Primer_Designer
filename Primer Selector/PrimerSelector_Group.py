...
degenerate_max_tm = 66 #The max Tm of degenerate primers, Celsius Degree
degenerate_min_tm = 44 #The min Tm of degenerate primers, Celsius Degree
max_degenerate_number = 128 #The max degeneration number of primer
...
rule_6_len = 8 #alignment score len
rule_6_cutoff = 8 #alignment cutoff score
...
def main():
    ...
    for primer_comb in itertools.combinations(primer_dict_atgc.keys(), 2): #use the primer_name of each primer, check if the two primers got an interaction
        if primer_check (primer_dict_atgc[primer_comb[0]], primer_dict_atgc[primer_comb[1]]) == False:
            ...
            primer_name_degenerate_0 = ('#').join(primer_comb[0].split('#')[:-1])
            primer_name_degenerate_1 = ('#').join(primer_comb[1].split('#')[:-1])
    ...
    for name in unusable_primers_dict.keys():
        degenerate_name = ('#').join(name.split('#')[:-1])
    ...
    with open(inputfile.split(".")[0]+'_primer_combinations.txt','w') as fhand:
        for product in itertools.product(*group_name_dict.values()):
            ...
            # Disable the function of amplicon length check in this version
            # if amplicon_length_check(70,550,10,primer_tuple) == False:
            #     flag = True #Amplicon length is inappropriate
            # if flag == False: 
            for comb in itertools.combinations(primer_list,2):
                if confliction_table.loc[comb[0],comb[1]] >=1 or confliction_table.loc[comb[1],comb[0]] >=1:
