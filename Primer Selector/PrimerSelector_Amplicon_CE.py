...
#modification: 20171010 Add Adjusted DegNumber properties to DegeneratePrimer, caluclate the ADegNum according to the position of the degenerate nucleotide
...
degenerate_max_tm = 75 #The max Tm of degenerate primers, Celsius Degree
degenerate_min_tm = 42 #The min Tm of degenerate primers, Celsius Degree
max_degenerate_number = 64 #The max degeneration number of primer
...
rule_6_len = 10 #alignment score len (min length)
rule_6_cutoff = 10 #alignment cutoff score
rule_7_len = 8 #delta G calculation min len
...
amplicon_len_max = 500 #The maximum length of amplicon
amplicon_len_min = 70 #The minimum length of amplicon
amplicon_lendiff_min = 15 #The minimum length differences between amplicons
...
def amplicon_length_check(amplicon_min,amplicon_max,amplicon_diff,primer_tuple, position_dict, primer_amplicon_match_dict):
    ...
    for comb in itertools.combinations(primer_tuple,2):
        if position_dict[comb[0]].ref_name == position_dict[comb[1]].ref_name: #the same organism
            ...
            amplicon_len = reverse_position - forward_position + 1
...
def primer_combinations(primer_combinations_file, group_name_dict, confliction_table, primer_amplicon_match_dict, position_dict, primer_dict):
    '''
...
def main():
    ...
    primer_combination_file = primer_file.split(".")[0]+'_primer_combinations.txt'
    primer_combinations(primer_combination_file, group_name_dict, confliction_table, primer_amplicon_match_dict, position_dict, primer_dict)
